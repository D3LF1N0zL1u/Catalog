                                          ...
                                          const char *updates_json)
{
    ...
    PG_TRY();
    {
        ...
        rc = ICEBERG_SPI_EXECUTE_WITH_ARGS(
            "SELECT jsonb_typeof($1::jsonb) = 'array', "
            "       jsonb_typeof($2::jsonb) = 'object', "
            "       EXISTS ("
            "           SELECT 1 "
            "           FROM jsonb_array_elements_text($1::jsonb) AS r(key) "
            "           JOIN jsonb_object_keys($2::jsonb) AS u(key) USING (key)"
            "       )",
            2, &argtypes[1], &values[1], NULL, true, 1);
        if (rc != SPI_OK_SELECT || SPI_processed != 1)
            ereport(ERROR,
                    (errcode(ERRCODE_INTERNAL_ERROR),
                     errmsg("failed to validate namespace property updates")));

        {
            bool isnull;
            bool removals_is_array = DatumGetBool(
                SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
            bool updates_is_object = DatumGetBool(
                SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 2, &isnull));
            bool has_overlap = DatumGetBool(
                SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 3, &isnull));

            if (!removals_is_array)
                ereport(ERROR,
                        (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                         errmsg("removals_json must be a JSON array")));
            if (!updates_is_object)
                ereport(ERROR,
                        (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                         errmsg("updates_json must be a JSON object")));
            if (has_overlap)
                ereport(ERROR,
                        (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
        ...
        rc = ICEBERG_SPI_EXECUTE_WITH_ARGS(
            ...
            "    SELECT "
            "      COALESCE(("
            "        SELECT json_agg(key ORDER BY key)::jsonb "
            "        FROM jsonb_object_keys($3::jsonb) AS u(key)"
            "      ), '[]'::jsonb) AS updated, "
            "      COALESCE(("
            "        SELECT json_agg(key ORDER BY key)::jsonb "
            "        FROM jsonb_array_elements_text($2::jsonb) AS r(key), locked "
            "        WHERE locked.properties ? r.key"
            "      ), '[]'::jsonb) AS removed, "
            "      COALESCE(("
            "        SELECT json_agg(key ORDER BY key)::jsonb "
            "        FROM jsonb_array_elements_text($2::jsonb) AS r(key), locked "
            "        WHERE NOT (locked.properties ? r.key)"
            "      ), '[]'::jsonb) AS missing"
            "), updated_row AS ("
            "    UPDATE iceberg_catalog.namespaces n "
            "    SET properties = ("
            "        SELECT COALESCE(json_object_agg(e.key, e.value)::jsonb, '{}'::jsonb) "
            "        FROM locked, jsonb_each(locked.properties) AS e(key, value) "
            "        WHERE NOT EXISTS ("
            "            SELECT 1 "
            ...
            "        )"
            "    ) || $3::jsonb "
            "    WHERE n.catalog_name = current_database()::text "
            "      AND n.namespace = $1 "
            "    RETURNING 1"
            ") "
            ...
            ")::text "
            "FROM response, updated_row",
            3, argtypes, values, NULL, false, 1);

        if (rc != SPI_OK_SELECT)
            ereport(ERROR,
                    (errcode(ERRCODE_INTERNAL_ERROR),
                     errmsg("metadata update namespace properties query failed")));
        ...
        {
            char *json_text = SPI_getvalue(SPI_tuptable->vals[0],
                                           SPI_tuptable->tupdesc, 1);
            MemoryContext old_context = MemoryContextSwitchTo(caller_context);
            result = pstrdup(json_text);
            MemoryContextSwitchTo(old_context);
        }
...
static char *
page_token_encode_namespace_next(const char *last_namespace)
{
    StringInfoData buf;
    char *encoded;
                              ...
                              const char *page_token)
{
    ...
    MemoryContext caller_context = CurrentMemoryContext;
    const char *parent_name = parent == NULL ? "" : parent;
    char *last_namespace_cursor;
    const char *token_err;
    bool has_parent = parent_name[0] != '\0';
    bool spi_connected = false;
    int total;
    int i;
    ...
    PG_TRY();
    {
        ...
        if (has_parent)
        {
            Datum ns_val[1] = {CStringGetTextDatum(parent_name)};
            ...
            if (ns_rc != SPI_OK_SELECT || SPI_processed == 0)
                ereport(ERROR,
                        (errcode(ERRCODE_UNDEFINED_OBJECT),
                         errmsg("namespace \"%s\" does not exist", parent_name)));
        }

        {
            ...
            query_values[0] = CStringGetTextDatum(parent_name);
            query_values[1] = CStringGetTextDatum(last_namespace_cursor);
            query_values[2] = Int32GetDatum(page_size + 1);

            if (has_parent)
                query_sql =
                    "SELECT child_namespace "
                    "FROM ("
                    "    SELECT DISTINCT split_part("
                    "        substr(namespace, length($1) + 2), '.', 1"
                    "    ) AS child_namespace "
                    "    FROM iceberg_catalog.namespaces "
                    "    WHERE catalog_name = current_database()::text "
                    "      AND namespace LIKE ($1 || '.%')"
                    ") s "
                    "WHERE ($2 = '' OR child_namespace > $2) "
                    "ORDER BY child_namespace ASC "
                    "LIMIT $3";
            else
                query_sql =
                    "SELECT namespace "
        ...
        {
            char *namespace_part = SPI_getvalue(SPI_tuptable->vals[i],
                                                SPI_tuptable->tupdesc, 1);
            StringInfoData escaped_part;

            initStringInfo(&escaped_part);
            escape_json(&escaped_part, namespace_part);
            ...
            if (has_parent)
            {
                StringInfoData escaped_parent;

                initStringInfo(&escaped_parent);
                escape_json(&escaped_parent, parent_name);
                appendStringInfo(&result, "[%s,%s]",
                                 escaped_parent.data, escaped_part.data);
                pfree(escaped_parent.data);
            }
            else
            {
                appendStringInfo(&result, "[%s]", escaped_part.data);
            }

            pfree(escaped_part.data);
        }
        appendStringInfoChar(&result, ']');
        ...
        if (total > page_size)
        {
            char *last_returned = SPI_getvalue(
                ...
                SPI_tuptable->tupdesc, 1);
            char *next_token = page_token_encode_namespace_next(last_returned);
