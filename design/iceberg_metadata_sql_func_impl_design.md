...
3. // ★ 创建 FDW 外表所属的 openGauss schema
   // 失败（如 schema 名非法）→ ereport → 事务回滚，不产生残留
   DDL_CreateSchema(p_namespace)

4. // ★ 写元信息表（利用 PK 约束仲裁并发冲突，详见 8.15）
...
5. return {"namespace": [p_namespace],
           "properties": p_properties ? p_properties : {}}
```

**DDL_CreateSchema 作用**：`create_table` 通过 `DDL_CreateStorage` 为 Iceberg 表创建 delta 表和 FDW 外表，这些表必须存在于某个 openGauss schema 下。`create_namespace` 提前创建该 schema，确保后续 `create_table` 有归属位置。DDL 操作与 META 写入在同一事务中：任一失败则全部回滚。
...
2. // ★ 行锁 + 存在性 + 非空检查（FOR UPDATE 防并发创建新表）
   META.LockNamespaceForUpdate(p_namespace)
   // 锁内检查：不存在 → ereport(P0004)
   // 锁内检查：NamespaceHasTables → ereport(P0005)

3. // 删除 META 记录（缓存表由 ON DELETE CASCADE 自动清理）
   META.DeleteNamespace(p_namespace)

4. // 删除对应的 openGauss schema（清理 FDW 外表）
   // 与 META 操作在同一事务中，失败时全部回滚，无残留
   DDL_DropSchema(p_namespace)

5. return {"success": true}
```

**FOR UPDATE 必要性**：普通 `SELECT` 不加锁，并发 `create_table` 可能在"存在性检查通过"和"DELETE 执行"之间插入新表。`FOR UPDATE` 串行化 namespace 行访问：`drop_namespace` 持锁期间 `create_table` 的 `FOR SHARE` 被阻塞（`iceberg_meta_register_table` 中），反之亦然。在 READ COMMITTED 下仅靠无锁查询无法保证正确性。
...
3. DDL_DropStorage(p_namespace, p_table)

4. META.DeleteTable(p_namespace, p_table)
     // META 内部处理 ON DELETE CASCADE

5. // SDK 清理（best-effort，若 purge 支持则清理数据文件）
...
| 函数 | 写顺序 | 并发机制 | 安全？ |
|------|--------|---------|--------|
| `create_namespace` | **DDL CreateSchema → META INSERT** | DDL 与 META INSERT 在同一事务中，任一失败全部回滚。`CREATE SCHEMA` 在系统目录上获取锁串行化并发请求，META INSERT 的 PK `(namespace)` 提供额外仲裁 | ✅ |
| `drop_namespace` | **META LockNS(FOR UPDATE) → META DELETE → DDL DropSchema** | FOR UPDATE 锁串行化 namespace 操作，防止并发 create_table。所有操作在同一事务中，任一失败全部回滚 | ✅ |
...
| `drop_table` | **DDL DropStorage → META DeleteTable → (best-effort) SDK DropTable** | DDL 事务性 + META DELETE 串行化。SDK 为 best-effort | ✅ |
...
| 函数 | 失败场景 | 结果 |
|------|---------|------|
| `create_namespace` | DDL CreateSchema 成功 → META INSERT 冲突 | ✅ 事务回滚，schema 自动撤销（DDL 事务性），无残留 |
| `create_namespace` | DDL CreateSchema 失败 | ✅ 事务回滚，META 尚未写入，无残留 |
| `drop_namespace` | 任一步骤失败 | ✅ 同一事务，全部回滚，无残留 |
